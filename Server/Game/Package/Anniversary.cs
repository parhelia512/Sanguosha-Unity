...
namespace SanguoshaServer.Package
{
    public class Anniversary : GeneralPackage
    {
        public Anniversary() : base("Anniversary")
        {
            ...
            related_skills = new Dictionary<string, List<string>>
            {
                ...
                { "tongye", new List<string>{ "#tongye-tm", "#tongye-range", "#tongye-max" } },
                { "changqu", new List<string>{ "#changqu" } },
            };
        }
    }
    ...
    public class Xialei : TriggerSkill
    {
        public Xialei() : base("xialei")
        {
        ...
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move &&
                (move.From_places.Contains(Place.PlaceHand) || move.From_places.Contains(Place.PlaceEquip)) && move.To_place == Place.PlaceTable && base.Triggerable(move.From, room))
            {
                for (int i = 0; i < move.Card_ids.Count; i++)
                {
                    int card_id = move.Card_ids[i];
                    if (room.GetCardPlace(card_id) == Place.PlaceTable && (move.From_places[i] == Place.PlaceHand || move.From_places[i] == Place.PlaceEquip) && WrappedCard.IsRed(room.GetCard(card_id).Suit))
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.To_place == Place.DiscardPile && base.Triggerable(move.From, room) && move.From.GetMark(Name) < 3)
            {
                for (int i = 0; i < move.Card_ids.Count; i++)
                {
                    int card_id = move.Card_ids[i];
                    if (room.GetCardPlace(card_id) == Place.DiscardPile && WrappedCard.IsRed(room.GetCard(card_id).Suit)
                        && (move.From_places[i] == Place.PlaceTable && room.GetCard(card_id).HasFlag(Name) || move.From_places[i] == Place.PlaceHand || move.From_places[i] == Place.PlaceEquip))
                        return new TriggerStruct(Name, move.From);
                }
            }
            return new TriggerStruct();
        ...
        {
            int count = 4 - player.GetMark(Name);
            if (count == 1)
            {
                room.DrawCards(player, 1, Name);
            }
            else
            {
                ...
                List<int> ids = room.NotifyChooseCards(player, card_ids, Name, 1, 1, "@xialei", string.Join("#", patterns), info.SkillPosition);
                card_ids.RemoveAll(t => ids.Contains(t));
                room.ReturnToDrawPile(card_ids, true, player);
                ...
                room.SendLog(b, player);
                room.ObtainCard(player, ref ids, new CardMoveReason(MoveReason.S_REASON_GOTCARD, player.Name, Name, string.Empty), false);
            }

            return false;
        }
    }
    public class XialeiClear : TriggerSkill
    {
        public XialeiClear() : base("#xialei")
        {
            events.Add(TriggerEvent.CardsMoveOneTime);
        }
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            ...
            if (move.From != null && move.To_place == Place.DiscardPile && move.From_places.Contains(Place.PlaceTable))
            {
                for (int i = 0; i < move.Card_ids.Count; i++)
                {
                    ...
                    if (move.From_places[i] == Place.PlaceTable && card.HasFlag("xialei"))
                        card.SetFlags("-xialei");
                }
            }
        }
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data) => new List<TriggerStruct>();
    ...
    public class Anzhi : TriggerSkill
    {
        public Anzhi() : base("anzhi")
        {
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.To_place == Place.DiscardPile && room.Current != null)
            {
                List<int> ids = room.ContainsTag(Name) ? (List<int>)room.GetTag(Name) : new List<int>();
                foreach (int id in move.Card_ids)
                {
                    if (room.GetCardPlace(id) == Place.DiscardPile && !ids.Contains(id))
                        ids.Add(id);
                }
                room.SetTag(Name, ids);
            }
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.Damaged && base.Triggerable(player, room) && !player.HasFlag(Name))
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            room.AskForUseCard(player, RespondType.Skill, "@@anzhi", "@anzhi-judge", null, -1, HandlingMethod.MethodUse, true, info.SkillPosition);
            return new TriggerStruct();
        }
    }
    ...
    public class AnzhiCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            {
                player.SetMark("xialei", 0);
                LogMessage log = new LogMessage("#xialei-reset")
                {
                    From = player.Name,
                    Arg = "xialei",
                };
            ...
            {
                player.SetFlags("anzhi");
                if (room.ContainsTag("anzhi") && room.GetTag("anzhi") is List<int> ids)
                {
    ...
    public class Caizhuang : ViewAsSkill
    {
        public Caizhuang() : base("caizhuang") { }
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.HasUsed(CaizhuangCard.ClassName) && !player.IsNude();
        public override bool ViewFilter(Room room, List<WrappedCard> selected, WrappedCard to_select, Player player)
        {
            if (RoomLogic.CanDiscard(room, player, player, to_select.Id))
                return selected.Find(t => t.Suit == to_select.Suit) == null;
        ...
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            if (cards.Count > 0)
            {
                WrappedCard cz = new WrappedCard(CaizhuangCard.ClassName) { Skill = Name };
                cz.AddSubCards(cards);
                return cz;
            }
            return null;
        }
    ...
    public class CaizhuangCard : SkillCard
    {
        public static string ClassName = "CaizhuangCard";
        public CaizhuangCard() : base(ClassName)
        {
            target_fixed = true;
            will_throw = true;
        }
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From;
            List<WrappedCard.CardSuit> suits = new List<WrappedCard.CardSuit>();
            while (player.Alive && suits.Count < card_use.Card.SubCards.Count)
            {
                room.DrawCards(player, 1, "caizhuang");
                suits.Clear();
                foreach (int id in player.GetCards("h"))
                {
                    WrappedCard card = room.GetCard(id);
    ...
    public class Huayi : TriggerSkill
    {
        public Huayi() : base("huayi")
        {
    ...
    public class HuayiEffect :  TriggerSkill
    {
    ...
    public class QiangzhiZH : ViewAsSkill
    {
        public QiangzhiZH() : base("qiangzhi_zh") { }
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.HasUsed(QiangzhiCard.ClassName);
        public override bool ViewFilter(Room room, List<WrappedCard> selected, WrappedCard to_select, Player player) => selected.Count < 3 && RoomLogic.CanDiscard(room, player, player, to_select.Id);
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            WrappedCard qz = new WrappedCard(QiangzhiCard.ClassName) { Skill = Name };
            qz.AddSubCards(cards);
            return qz;
        }
    }

    public class QiangzhiCard : SkillCard
    {
        public static string ClassName = "QiangzhiCard";
        public QiangzhiCard() : base(ClassName)
        {
            will_throw = false;
        }

        public override bool TargetFilter(Room room, List<Player> targets, Player to_select, Player Self, WrappedCard card) => targets.Count == 0 && to_select != Self && RoomLogic.CanDiscard(room, Self, to_select, "he") && card.SubCards.Count + to_select.GetCardCount(true) >= 3;
        public override void Use(Room room, CardUseStruct card_use)
        {
            List<int> self = new List<int>(card_use.Card.SubCards);
            int count = 3 - self.Count;
            Player player = card_use.From;
            Player target = card_use.To[0];
            List<CardsMoveStruct> moves1 = new List<CardsMoveStruct>();
            if (self.Count > 0)
            {
                CardsMoveStruct _move = new CardsMoveStruct(self, player, Place.PlaceTable, new CardMoveReason(MoveReason.S_REASON_THROW, player.Name, "qiangzhi_zh", string.Empty))
                {
                    From = player.Name
                };
                moves1.Add(_move);
            ...
            if (count > 0)
            {
                ids = room.AskForCardsChosen(player, target, "he", "qiangzhi_zh", count, count, false, HandlingMethod.MethodDiscard);
                if (ids.Count > 0)
                {
                    CardsMoveStruct _move = new CardsMoveStruct(ids, target, Place.PlaceTable, new CardMoveReason(MoveReason.S_REASON_DISMANTLE, player.Name, target.Name, "qiangzhi_zh", string.Empty))
                    {
                        From = target.Name
                    };
                    moves1.Add(_move);
            ...
            room.MoveCardsAtomic(moves1, true);
            List<CardsMoveStruct> moves2 = new List<CardsMoveStruct>();
            if (self.Count > 0)
            {
                List<int> dis1 = room.GetCardIdsOnTable(self);
                if (dis1.Count > 0)
                {
                    CardsMoveStruct _move = new CardsMoveStruct(dis1, null, Place.DiscardPile, new CardMoveReason(MoveReason.S_REASON_THROW, player.Name, "qiangzhi_zh", string.Empty))
                    {
                        From = player.Name
                    };
                    moves2.Add(_move);
            ...
            if (count > 0)
            {
                List<int> dis2 = room.GetCardIdsOnTable(self);
                if (dis2.Count > 0)
                {
                    CardsMoveStruct _move = new CardsMoveStruct(dis2, null, Place.DiscardPile, new CardMoveReason(MoveReason.S_REASON_DISMANTLE, player.Name, target.Name, "qiangzhi_zh", string.Empty))
                    {
                        From = target.Name
                    };
                    moves2.Add(_move);
            ...
            if (moves2.Count > 0) room.MoveCardsAtomic(moves2, true);
            if (player.Alive && target.Alive)
            {
                if (count == 0)
                    room.Damage(new DamageStruct("qiangzhi_zh", player, target, 1));
                else if (count == 3)
                    room.Damage(new DamageStruct("qiangzhi_zh", target, player, 1));
            }
        }
    }

    public class Pitian : TriggerSkill
    {
        public Pitian() : base("pitian")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardsMoveOneTime, TriggerEvent.Damaged, TriggerEvent.EventPhaseStart };
            skill_type = SkillType.Replenish;
        }
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.To_place == Place.DiscardPile
                && (move.Reason.Reason & MoveReason.S_MASK_BASIC_REASON) == MoveReason.S_REASON_DISCARD && base.Triggerable(move.From, room))
            {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.EventPhaseStart && base.Triggerable(player, room) && player.Phase == PlayerPhase.Finish && player.HandcardNum < RoomLogic.GetMaxCards(room, player))
                return new TriggerStruct(Name, player);
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (room.AskForSkillInvoke(player, Name, "@pitian", info.SkillPosition))
            {
                room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
                return info;
            }
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            int count = Math.Min(5, RoomLogic.GetMaxCards(room, player) - player.HandcardNum);
            player.SetMark(Name, 0);
            room.RemovePlayerStringMark(player, Name);
            room.DrawCards(player, count, Name);
            return false;
    ...
    public class PitianMax : MaxCardsSkill
    {
        public PitianMax() : base("#pitian") { }
        public override int GetExtra(Room room, Player target) => RoomLogic.PlayerHasSkill(room, target, "pitian") ? target.GetMark("pitian") : 0;
    }

    public class Jiuxian : TriggerSkill
    {
        public Jiuxian() : base("jiuxian")
        {
            events.Add(TriggerEvent.Damage);
            frequency = Frequency.Compulsory;
            view_as_skill = new JiuxianVS();
        }

        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (data is DamageStruct damage && damage.Card != null && damage.Card.GetSkillName() == Name && player.Alive && player.HasFlag(Name) && !damage.Transfer && damage.To.Alive)
                return new TriggerStruct(Name, player) { SkillPosition = damage.Card.SkillPosition };
            return new TriggerStruct();
        }
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (data is DamageStruct damage)
            {
                ...
                foreach (Player p in room.GetOtherPlayers(damage.To))
                    if (RoomLogic.InMyAttackRange(room, damage.To, p) && p.IsWounded())
                        targets.Add(p);

                if (targets.Count > 0)
                {
                    Player target = room.AskForPlayerChosen(player, targets, Name, "@jiuxian:" + damage.To.Name, true, true, info.SkillPosition);
                    if (target != null)
                    {
                        player.SetTag(Name, target.Name);
                        return info;
    ...
    public class JiuxianVS : ViewAsSkill
    {
        public JiuxianVS() : base("jiuxian") { }
        public override bool ViewFilter(Room room, List<WrappedCard> selected, WrappedCard to_select, Player player)
        {
            int count = (int)Math.Ceiling((float)player.GetCardCount(false) / 2);
            return room.GetCardPlace(to_select.Id) == Place.PlaceHand && selected.Count < count;
        }
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.IsKongcheng() && !player.HasUsed(JiuxianCard.ClassName);
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            if (cards.Count == Math.Ceiling((float)player.GetCardCount(false) / 2))
            {
                WrappedCard jx = new WrappedCard(JiuxianCard.ClassName) { Skill = Name };
    ...
    public class JiuxianCard : SkillCard
    {
        public static string ClassName = "JiuxianCard";
        public JiuxianCard() : base(ClassName)
        { will_throw = false; }
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From;
            Player target = card_use.To[0];
            CardMoveReason reason = new CardMoveReason(MoveReason.S_REASON_RECAST, player.Name, "jiuxian", string.Empty);
            CardsMoveStruct move = new CardsMoveStruct(card_use.Card.SubCards, player, Place.PlaceTable, reason)
            {
                To_pile_name = string.Empty,
                From = player.Name
            };
            List<CardsMoveStruct> moves = new List<CardsMoveStruct> { move };
            room.MoveCardsAtomic(moves, true);
            room.BroadcastSkillInvoke("@recast", player.IsMale() ? "male" : "female", -1);

            int count = card_use.Card.SubCards.Count;
            List<int> table_cardids = room.GetCardIdsOnTable(card_use.Card.SubCards);
            ...
            if (player.Alive)
                room.DrawCards(player, count, "recast");

            if (player.Alive && target.Alive)
            {
                WrappedCard duel = new WrappedCard(Duel.ClassName) { Skill = "jiuxian" };
                if (RoomLogic.IsProhibited(room, player, target, duel) == null)
                {
                    player.SetFlags("jiuxian");
    ...
    public class Chenyong : TriggerSkill
    {
        public Chenyong() : base("chenyong")
        {
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if ((triggerEvent == TriggerEvent.CardUsed && player.Phase == PlayerPhase.Play && data is CardUseStruct use && !Engine.IsSkillCard(use.Card.Name))
                || (triggerEvent == TriggerEvent.CardResponded && data is CardResponseStruct resp && resp.Use && player.Phase == PlayerPhase.Play && !Engine.IsSkillCard(resp.Card.Name)))
            {
                List<CardType> types = player.ContainsTag(Name) ? (List<CardType>)player.GetTag(Name) : new List<CardType>();
                CardType type = CardType.TypeUnknow;
                if (triggerEvent == TriggerEvent.CardUsed && data is CardUseStruct _use)
                    type = Engine.GetFunctionCard(_use.Card.Name).TypeID;
                else if (triggerEvent == TriggerEvent.CardResponded && data is CardResponseStruct _resp)
                    type = Engine.GetFunctionCard(_resp.Card.Name).TypeID;
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (player.GetTag(Name) is List<CardType> types && room.AskForSkillInvoke(player, Name, string.Format("@chenyong:::{0}", types.Count), info.SkillPosition))
            {
                room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
                return info;
            }
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (player.GetTag(Name) is List<CardType> types)
            {
    ...
    public class Huizhi : TriggerSkill
    {
        public Huizhi() : base("huizhi")
        {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (base.Triggerable(player, room) && player.Phase == PlayerPhase.Draw && !player.IsKongcheng() && RoomLogic.CanDiscard(room, player, player, "h"))
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (room.AskForDiscard(player, Name, player.GetCardCount(false), 1, true, false, "@huizhi", true, info.SkillPosition))
            {
                room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
                return info;
            }
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            int count = 1;
            if (max > player.HandcardNum)
                count = Math.Min(max - player.HandcardNum, 5);
    ...
    public class Jijiao : TriggerSkill
    {
        public Jijiao() : base("jijiao")
        {
            skill_type = SkillType.Wizzard;
            frequency = Frequency.Limited;
            limit_mark = "@jijiao";
            events = new List<TriggerEvent> { TriggerEvent.CardsMoveOneTime, TriggerEvent.SwapPile, TriggerEvent.Death, TriggerEvent.EventPhaseChanging };
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.From != null && move.From.Alive && move.To_place == Place.DiscardPile)
            {
                ...
                if (cards.Count > 0)
                {
                    List<int> old = move.From.ContainsTag(Name) ? (List<int>)move.From.GetTag(Name) : new List<int>();
                    cards.RemoveAll(t => old.Contains(t));
                    if (cards.Count > 0)
                    {
                        old.AddRange(cards);
                        move.From.SetTag(Name, old);
                    }
                }
            }
            else if (triggerEvent == TriggerEvent.SwapPile || triggerEvent == TriggerEvent.Death)
            {
                foreach (Player p in room.GetAlivePlayers())
                    p.SetFlags(Name);
            }
        ...
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            ...
            {
                foreach (Player p in room.GetAlivePlayers())
                    if (base.Triggerable(p, room) && p.HasFlag(Name) && p.GetMark(limit_mark) == 0)
                        triggers.Add(new TriggerStruct(Name, p));
            }
            return triggers;
        }
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            room.SetPlayerMark(ask_who, limit_mark, 1);
            return false;
        }
    }

    public class JijiaoVS : ZeroCardViewAsSkill
    {
        public JijiaoVS() : base("jijiao") { }
        public override bool IsEnabledAtPlay(Room room, Player player) => player.GetMark("@jijiao") > 0 && player.ContainsTag(Name);
        public override WrappedCard ViewAs(Room room, Player player) => new WrappedCard(JijiaoCard.ClassName) { Skill = Name };
    }

    public class JijiaoCard : SkillCard
    {
        public static string ClassName = "JijiaoCard";
        public JijiaoCard() : base(ClassName) { }
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From;
            Player target = card_use.To[0];
            List<int> old = (List<int>)player.GetTag("jijiao"), cards = room.DiscardPile.FindAll(t => old.Contains(t));
            if (cards.Count > 0)
                room.ObtainCard(target, ref cards, new CardMoveReason(MoveReason.S_REASON_RECYCLE, player.Name, target.Name, "jijiao", string.Empty), true);
        }
    }

    public class Chanjuan : TriggerSkill
    {
        public Chanjuan() : base("chanjuan")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardFinished, TriggerEvent.CardUsed };
            view_as_skill = new ChanjuanVS();
        }
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.CardUsed && data is CardUseStruct use && use.Card.GetSkillName() == Name && player.Alive)
            {
                List<string> cards = player.ContainsTag("chuanjuan_cards") ? (List<string>)player.GetTag("chuanjuan_cards") : new List<string>();
                cards.Add(use.Card.Name);
                player.SetTag("chuanjuan_cards", cards);
                if (player.GetTag(string.Format("{0}_{1}", Name, use.Card.Name)) is List<string> targets && targets.Count == use.To.Count)
                {
                    bool check = true;
                    foreach (Player p in use.To)
                    {
                        if (!targets.Contains(p.Name) || use.To.Count(t => t == p) != targets.Count(t => t == p.Name))
                        {
                            check = false;
                            break;
                        }
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.CardFinished && data is CardUseStruct use && base.Triggerable(player, room) && (!player.ContainsTag("chuanjuan_cards") || (player.GetTag("chuanjuan_cards") is List<string> cards && !cards.Contains(use.Card.Name))))
            {
                FunctionCard fcard = Engine.GetFunctionCard(use.Card.Name);
                if (fcard.Name != Jink.ClassName && (fcard is BasicCard || fcard.IsNDTrick()))
                    return new TriggerStruct(Name, player);
            }
            return new TriggerStruct();
        }
    ...
    public class ChanjuanVS : ViewAsSkill
    {
        public ChanjuanVS() : base("chanjuan") { response_pattern = "@@chanjuan"; }
        public override bool ViewFilter(Room room, List<WrappedCard> selected, WrappedCard to_select, Player player) => false;
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            if (cards.Count == 1 && cards[0].IsVirtualCard())
        ...
        public override List<WrappedCard> GetGuhuoCards(Room room, List<WrappedCard> cards, Player player)
        {
            ...
            WrappedCard slash = new WrappedCard((string)player.GetTag(Name)) { Skill = Name };
            result.Add(slash);
            return result;
        }
    }

    public class Xunbie : TriggerSkill
    {
        public Xunbie() : base("xunbie")
        {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player target, ref object data, Player ask_who)
        {
            if (data is DyingStruct dying && dying.Who == target && target.Hp <= 0
                && base.Triggerable(target, room) && target.GetMark(limit_mark) > 0)
        ...
        {
            player.SetFlags(Name);
            List<string> generals = new List<string> { "ganfuren", "mifuren" };
            foreach (Player p in room.GetOtherPlayers(player))
                generals.Remove(p.General1);
            ...
            if (generals.Count > 0)
            {
                ...
                foreach (string skill_name in Engine.GetGeneralSkills(general_name, room.Setting.GameMode, true))
                {
                    ...
                    object new_data = new InfoStruct() { Info = skill_name, Head = true };
                    room.RoomThread.Trigger(TriggerEvent.EventAcquireSkill, room, player, ref new_data);
                }
    ...
    public class XunbieEffect : TriggerSkill
    {
        public XunbieEffect() : base("#xunbie")
        {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (player.HasFlag("xunbie")) return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }

        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            room.SendCompulsoryTriggerLog(player, "xunbie");
            return true;
        }
    }
    public class ShushenClassic : TriggerSkill
    {
        public ShushenClassic() : base("shushen_classic")
        {
            events.Add(TriggerEvent.HpRecover);
            skill_type = SkillType.Replenish;
        }
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            Player target = room.AskForPlayerChosen(player, room.GetOtherPlayers(player), Name, "@shushen", true, true, info.SkillPosition);
            if (target != null)
            {
                room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (to != null)
            {
                ...
                if (to.IsWounded()) choices.Add("recover");
                string choice = room.AskForChoice(player, Name, choices, null, to, info.SkillPosition);
                if (choice == "draw")
    ...
    public class ShenzhiClassic : PhaseChangeSkill
    {
        public ShenzhiClassic() : base("shenzhi_classic")
        {
            skill_type = SkillType.Recover;
        }
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
            => base.Triggerable(player, room) && player.Phase == PlayerPhase.Start && player.IsWounded() && player.HandcardNum > player.Hp ? new TriggerStruct(Name, player) : new TriggerStruct();
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (room.AskForDiscard(player, Name, 1, 1, true, false, "@shenzhi", true, info.SkillPosition))
            {
                room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
                return info;
            }
        ...
        {
            if (ganfuren.Alive) room.Recover(ganfuren);
            return false;
        }
    }
    ...
    public class Tongye : TriggerSkill
    {
        public Tongye() : base("tongye")
        {
            events = new List<TriggerEvent> { TriggerEvent.GameStart, TriggerEvent.Death, TriggerEvent.EventPhaseProceeding };
            frequency = Frequency.Compulsory;
        }
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            List<TriggerStruct> triggers = new List<TriggerStruct>();
            if (triggerEvent == TriggerEvent.GameStart || triggerEvent == TriggerEvent.Death)
            {
                foreach (Player p in room.GetAlivePlayers())
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            else
            {
                ...
                foreach (Player p in room.GetAlivePlayers())
                    p.SetMark(Name, kingdoms.Count);
            }

            return false;
        }
    }
    public class TongyeTM : TargetModSkill
    {
        public TongyeTM() : base("#tongye-tm") { }
        public override int GetResidueNum(Room room, Player from, WrappedCard card) => RoomLogic.PlayerHasSkill(room, from, "tongye") && from.GetMark("tongye") <= 2 ? 3 : 0;
    }
    public class TongyeMax : MaxCardsSkill
    {
        public TongyeMax() : base("#tongye-max") { }
        public override int GetExtra(Room room, Player target) => RoomLogic.PlayerHasSkill(room, target, "tongye") && target.GetMark("tongye") <= 4 ? 3 : 0;
    }
    public class TongyeRange : AttackRangeSkill
    {
        public TongyeRange() : base("#tongye-range") { }
        public override int GetExtra(Room room, Player target, bool include_weapon) => RoomLogic.PlayerHasSkill(room, target, "tongye") && target.GetMark("tongye") <= 3 ? 3 : 0;
    }
    public class Changqu : ZeroCardViewAsSkill
    {
        public Changqu() : base("changqu") {}
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.HasUsed(ChangquCard.ClassName);
        public override WrappedCard ViewAs(Room room, Player player) => new WrappedCard(ChangquCard.ClassName) { Skill = Name };
    }
    public class ChangquCard : SkillCard
    {
        public static string ClassName = "ChangquCard";
        public ChangquCard() : base(ClassName) { }
        public override bool TargetFilter(Room room, List<Player> targets, Player to_select, Player Self, WrappedCard card) => targets.Count == 0 && to_select != Self;
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player target = card_use.To[0];
            Player player = card_use.From;
            List<Player> targets = new List<Player>();
            if (room.GetNextAlive(player) == target && room.GetLastAlive(player) == target)
            {
                targets.Add(target);
            }
            else
            {
                string choice = room.AskForChoice(player, "changqu", "last+next", new List<string> { "@to-player:" + card_use.From.Name }, target, card_use.Card.SkillPosition);
                if (choice == "last")
                {
                    Player last = room.GetLastAlive(player);
                    while (last != target)
                    {
                        targets.Add(last);
                        last = room.GetNextAlive(last);
                    }
                    targets.Add(target);
                }
                else
                {
                    Player next = room.GetNextAlive(player);
                    while (next != target)
                    {
                        targets.Add(next);
            ...
            if (targets.Count > 0)
            {
                ...
                foreach (Player p in targets)
                {
                    if (p.Alive)
                    {
                        ...
                        bool give = false;
                        if (p.HandcardNum >= num && player.Alive)
                        {
                            List<int> ids = room.AskForExchange(p, "changqu", num, num, string.Format("@changqu:::{0}", num), string.Empty, ".", string.Empty);
                            if (ids.Count == num)
                            {
                                room.ObtainCard(player, ref ids, new CardMoveReason(MoveReason.S_REASON_GIVE, p.Name, player.Name, string.Empty), false);
                                give = true;
                                count++;
                                break;
                            }
                        ...
                        if (!give)
                        {
                            p.AddMark("changqu", num);
                            room.SetPlayerStringMark(p, "changqu", p.GetMark("changqu").ToString());
                            if (!p.Chained && RoomLogic.CanBeChainedBy(room, p, p)) room.SetPlayerChained(p, true);
                        }
                    }
                }
            }
    ...
    public class ChangquEffect : TriggerSkill
    {
        public ChangquEffect() : base("#changqu")
        {
            events.Add(TriggerEvent.DamageInflicted);
            frequency = Frequency.Compulsory;
        }
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (data is DamageStruct damage && player.GetMark("changqu") > 0 && damage.Nature != DamageStruct.DamageNature.Normal)
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            DamageStruct damage = (DamageStruct)data;
            damage.Damage += player.GetMark("changqu");
            player.SetMark("changqu", 0);
            room.RemovePlayerStringMark(player, "changqu");
    ...
    public class YuanyuZYEffect : TriggerSkill
    {
        ...
        {
            events = new List<TriggerEvent> { TriggerEvent.EventPhaseStart, TriggerEvent.Damage };
            frequency = Frequency.Compulsory;
        }
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            List<TriggerStruct> triggers = new List<TriggerStruct>();
            if ((triggerEvent == TriggerEvent.Damage || triggerEvent == TriggerEvent.EventPhaseStart && player.Phase == PlayerPhase.Discard) && player.Alive
                && player.ContainsTag("yuanyu_zy") && player.GetTag("yuanyu_zy") is List<string> names && !player.IsKongcheng())
            {
                int count = 1;
                if (triggerEvent == TriggerEvent.Damage && data is DamageStruct damage) count = damage.Damage;
                foreach (string player_name in names)
                {
                    Player target = room.FindPlayer(player_name);
                    if (target != null && RoomLogic.PlayerHasSkill(room, target, "yuanyu_zy"))
                        triggers.Add(new TriggerStruct(Name, player, target) { Times = count });
                }
            }

            if (triggerEvent == TriggerEvent.EventPhaseStart && player.Phase == PlayerPhase.Discard && base.Triggerable(player, room) && player.GetPile("yuanyu_zy").Count > 0)
            {
                foreach (Player p in room.GetOtherPlayers(player))
                {
                    if (p.ContainsTag("yuanyu_zy") && p.GetTag("yuanyu_zy") is List<string> _names && !p.IsKongcheng())
                    {
                        foreach (string player_name in _names)
                        {
                            if (player_name == player.Name)
                                triggers.Add(new TriggerStruct(Name, p, player));
                        }
                    }
                }
            }
    ...
    public class Xiyan : TriggerSkill
    {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (ask_who.Alive && room.Current != null && room.Current.Phase != PlayerPhase.NotActive)
            {
                ...
                {
                    ask_who.ClearHistory(YuanyuZYCard.ClassName);
                    ask_who.AddMark(Name);
                    log.Arg = "+4";
                    room.SendLog(log);
                }
                else if (room.AskForSkillInvoke(ask_who, Name, string.Format("@xiyan:{0}", room.Current.Name), info.SkillPosition))
                {
                    room.DoAnimate(AnimateType.S_ANIMATE_INDICATE, ask_who.Name, room.Current.Name);
    ...
    public class Cuichuan : OneCardViewAsSkill
    {
        public Cuichuan() : base("cuichuan") { filter_pattern = ".!"; }
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.IsKongcheng() && !player.HasUsed(CuichuanCard.ClassName);
        public override WrappedCard ViewAs(Room room, WrappedCard card, Player player)
        {
            WrappedCard cc = new WrappedCard(CuichuanCard.ClassName) { Skill = Name };
            cc.AddSubCard(card);
            return cc;
        }
    }

    public class CuichuanCard : SkillCard
    {
        public static string ClassName = "CuichuanCard";
        public CuichuanCard() : base(ClassName)
        {
            will_throw = true;
        }
        public override bool TargetFilter(Room room, List<Player> targets, Player to_select, Player Self, WrappedCard card)
        {
            if (targets.Count == 0)
            {
                for (int i =0;i <5;i++)
                {
                    if (to_select.CanPutEquip(i) && !to_select.HasEquip(i))
                        return true;
                }
            }
            return false;
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (target.CanPutEquip(i) && !target.HasEquip(i))
                    indexes.Add(i);
            }
            ...
            foreach (int id in room.DrawPile)
            {
                WrappedCard card = room.GetCard(id);
                FunctionCard fcard = Engine.GetFunctionCard(card.Name);
                if (fcard is EquipCard equip && indexes.Contains((int)equip.EquipLocation()))
                {
                    card_id = id;
                    break;
                }
            ...
            if (card_id != -1)
            {
                ...
                room.MoveCardTo(room.GetCard(card_id), player, target, Place.PlaceEquip, new CardMoveReason(MoveReason.S_REASON_PUT, player.Name, target.Name, "cuichuan", null));
                LogMessage log = new LogMessage
                {
                    Type = "$ZhijianEquip",
                    From = target.Name,
    ...
    public class Zhengxu : TriggerSkill
    {
        public Zhengxu() : base("zhengxu")
        {
            events = new List<TriggerEvent> { TriggerEvent.Damaged, TriggerEvent.CardsMoveOneTime, TriggerEvent.DamageDefined };
        }
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
        ...
        {
            List<TriggerStruct> triggers = new List<TriggerStruct>();
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.From != null && (move.From_places.Contains(Place.PlaceEquip) || move.From_places.Contains(Place.PlaceHand))
                && move.From.HasFlag("zhengxu_draw") && !move.From.HasFlag("zhengxu_invoke_draw") && base.Triggerable(move.From, room))
                triggers.Add(new TriggerStruct(Name, move.From));
            else if (triggerEvent == TriggerEvent.DamageDefined && !player.HasFlag("zhengxu_invoke_invalid") && player.HasFlag("zhengxu_invalid") && base.Triggerable(player, room))
                triggers.Add(new TriggerStruct(Name, player));
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            {
                if (room.AskForSkillInvoke(player, Name, string.Format("@zhengxu-damage:::{0}", damage.Damage), info.SkillPosition))
                {
                    player.SetFlags("zhengxu_invoke_invalid");
                    room.BroadcastSkillInvoke(Name, ask_who, info.SkillPosition);
                    return info;
                }
                player.SetFlags("-zhengxu_invalid");
            }
            return new TriggerStruct();
        }
    ...
    public class ZhengxuRecord : TriggerSkill
    {
        public ZhengxuRecord() : base("#zhengxu")
        {
        ...
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.From != null && (move.From_places.Contains(Place.PlaceEquip) || move.From_places.Contains(Place.PlaceHand))
                && move.From.HasFlag("zhengxu_draw"))
                move.From.SetFlags("-zhengxu_draw");
            else if (triggerEvent == TriggerEvent.DamageDefined && player != null && player.HasFlag("zhengxu_invalid"))
                player.SetFlags("zhengxu_invalid");
        }
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data) => new List<TriggerStruct>();
    }

    public class Zuojian : TriggerSkill
    {
        public Zuojian() : base("zuojian")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardUsed, TriggerEvent.CardResponded, TriggerEvent.EventPhaseEnd, TriggerEvent.EventPhaseChanging };
            skill_type = SkillType.Wizzard;
        }
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.EventPhaseEnd && player.Phase == PlayerPhase.Play && player.GetMark(Name) > player.Hp && base.Triggerable(player, room))
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            List<Player> discards = new List<Player>();
            int count = player.GetCards("e").Count;
            foreach (Player p in room.GetOtherPlayers(player))
            {
                int _count = p.GetCards("e").Count;
                if (_count > count)
                    drawers.Add(p);
                else if (_count < count && !p.IsKongcheng() && RoomLogic.CanDiscard(room, player, p, "h"))
                    discards.Add(p);
            }
            List<string> choices = new List<string>();
            if (drawers.Count > 0)
            ...
            {
                room.SortByActionOrder(ref discards);
                foreach (Player p in discards)
                    room.DoAnimate(AnimateType.S_ANIMATE_INDICATE, player.Name, p.Name);
                foreach (Player p in discards)
                {
                    if (player.Alive && p.Alive && !p.IsKongcheng() && RoomLogic.CanDiscard(room, player, p, "h"))
                    {
                        int id = room.AskForCardChosen(player, p, "h", Name, false, HandlingMethod.MethodDiscard);
                        room.ThrowCard(id, p, player);
                    }
                }
            }
    ...
    public class Yuandi : TriggerSkill
    {
        public Yuandi() : base("yuandi")
        {
            events = new List<TriggerEvent> { TriggerEvent.EventPhaseChanging, TriggerEvent.CardUsed };
        }
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.EventPhaseChanging && data is PhaseChangeStruct change && change.From == PlayerPhase.Play)
                player.SetMark(Name, 0);
            else if (triggerEvent == TriggerEvent.CardUsed && data is CardUseStruct use && player != null && player.Alive && player.Phase == PlayerPhase.Play && !Engine.IsSkillCard(use.Card.Name))
                player.AddMark(Name);
        }
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (!player.IsKongcheng() && RoomLogic.CanDiscard(room, ask_who, player, "h")) choices.Insert(0, "discard");
            string choice = room.AskForChoice(ask_who, Name, choices, new List<string> { "@to-player:" + player.Name }, player, info.SkillPosition);
            if (choice != "cancel")
            {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (ask_who.GetTag(Name) is string choice)
            {
                ask_who.RemoveTag(Name);
                if (choice == "discard")
                {
                    int card_id = room.AskForCardChosen(ask_who, player, "h", Name, false, HandlingMethod.MethodDiscard);
                    List<int> ids = new List<int> { card_id };
                    room.ThrowCard(ref ids, new CardMoveReason(MoveReason.S_REASON_DISMANTLE, ask_who.Name, player.Name, Name, string.Empty), ask_who);
                }
                else
                {
                    room.DrawCards(player, new DrawCardStruct(1, ask_who, Name));
    ...
    public class Xinyou : ZeroCardViewAsSkill
    {
        public Xinyou() : base("xinyou") { }
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.HasUsed(XinyouCard.ClassName);
        public override WrappedCard ViewAs(Room room, Player player) => new WrappedCard(XinyouCard.ClassName) { Skill = Name };
    }

    public class XinyouCard : SkillCard
    {
        public static string ClassName = "XinyouCard";
        public XinyouCard() : base(ClassName) { target_fixed = true; }
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From;
            if (player.IsWounded())
            {
                player.SetFlags("xinyou_discard");
            ...
            {
                int count = player.MaxHp - player.HandcardNum;
                if (count > 1) player.SetFlags("xinyou_lose");
                room.DrawCards(player, count, "xinyou");
            }
        }
    }

    public class XinyouEffect : TriggerSkill
    {
        public XinyouEffect() : base("#xinyou")
        {
            events.Add(TriggerEvent.EventPhaseStart);
            frequency = Frequency.Compulsory;
        }
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (player.Phase == PlayerPhase.Finish && (player.HasFlag("xinyou_lose") || player.HasFlag("xinyou_discard")))
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (player.HasFlag("xinyou_lose"))
                room.LoseHp(player);

            if (player.Alive && player.HasFlag("xinyou_discard") && !player.IsNude())
                room.AskForDiscard(player, "xinyou", 2, 2, false, true, "@xinyou", false, info.SkillPosition);
            return false;
        }
    }

    public class Huiling : TriggerSkill
    {
        public Huiling() : base("huiling")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardResponded, TriggerEvent.CardUsed };
            frequency = Frequency.Compulsory;
        }
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if ((triggerEvent == TriggerEvent.CardUsed && data is CardUseStruct use && !Engine.IsSkillCard(use.Card.Name) && base.Triggerable(player, room))
                || (triggerEvent == TriggerEvent.CardResponded && data is CardResponseStruct resp && resp.Use && base.Triggerable(player, room)))
            {
                int black = 0, red = 0;
                foreach  (int id in room.DiscardPile)
                {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            else if (black > red)
            {
                ...
                if (targets.Count > 0)
                {
                    Player target = room.AskForPlayerChosen(player, targets, Name, "@huiling", true, true, info.SkillPosition);
                    if (target != null)
                    {
            ...
            if (player.Alive && (red < black && WrappedCard.IsRed(suit)) || (black < red && WrappedCard.IsBlack(suit)))
            {
                player.AddMark(Name);
                room.SetPlayerStringMark(player, Name, player.GetMark(Name));
            }

            return false;
        }
    }
    public class Chongxu :  ZeroCardViewAsSkill
    {
        public Chongxu() : base("chongxu")
        {
            limit_mark = "@chongxu";
            frequency = Frequency.Limited;
        }
        public override bool IsEnabledAtPlay(Room room, Player player) => player.GetMark(limit_mark) > 0 && player.GetMark("huiling") >= 4;
        public override WrappedCard ViewAs(Room room, Player player) => new WrappedCard(ChongxuCard.ClassName) { Skill = Name };
    }

    public class ChongxuCard : SkillCard
    {
        public static string ClassName = "ChongxuCard";
        public ChongxuCard() : base(ClassName)
        {
            target_fixed = true;
        }
        public override void OnUse(Room room, CardUseStruct card_use)
        {
            room.SetPlayerMark(card_use.From, "@chongxu", 0);
            room.BroadcastSkillInvoke("chongxu", card_use.From, card_use.Card.SkillPosition);
            room.DoSuperLightbox(card_use.From, card_use.Card.SkillPosition, "chongxu");
            base.OnUse(room, card_use);
        }
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From;
            int count = player.GetMark("huiling");
            player.SetMark("huiling", 0);
            room.RemovePlayerStringMark(player, "huiling");
            ...
            if (player.Alive)
                room.HandleAcquireDetachSkills(player, new List<string> { "-huiling", "taji", "qinghuang" }, false, true);
        }
    }
    public class Taji : TriggerSkill
    {
        public Taji() : base("taji")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardsMoveOneTime, TriggerEvent.DamageCaused };
            frequency = Frequency.Compulsory;
        }
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && base.Triggerable(move.From, room) && (move.From_places.Contains(Place.PlaceHand) || move.From_places.Contains(Place.PlaceEquip)))
                return new TriggerStruct(Name, move.From);
            else if (triggerEvent == TriggerEvent.DamageCaused && data is DamageStruct damage && player.GetMark(Name) > 0 && damage.To != player)
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move)
            {
                ...
                if ((move.Reason.Reason & MoveReason.S_MASK_BASIC_REASON) == MoveReason.S_REASON_USE)
                    random.Add(0);
                else if (move.Reason.Reason == MoveReason.S_REASON_RESPONSE)
                    random.Add(1);
                else if ((move.Reason.Reason & MoveReason.S_MASK_BASIC_REASON) == MoveReason.S_REASON_DISCARD)
                    random.Add(2);
                else
                    random.Add(3);
                if (ask_who.HasFlag("qinghuang"))
                {
                ...
                foreach (int id in random)
                {
                    if (id == 0)
                    {
                        List<Player> targets = new List<Player>();
                        foreach (Player p in room.GetOtherPlayers(ask_who))
                            if (!p.IsNude() && RoomLogic.CanDiscard(room, ask_who, p, "he"))
                                targets.Add(p);
                        if (targets.Count > 0)
                        {
                            Player target = room.AskForPlayerChosen(ask_who, targets, Name, "@taji", true, true, info.SkillPosition);
                            if (target != null)
                            {
                                int card_id = room.AskForCardChosen(ask_who, target, "he", Name, false, HandlingMethod.MethodDiscard);
                    ...
                    {
                        if (ask_who.IsWounded()) room.Recover(ask_who);
                    }
                    else
                    {
                        ask_who.AddMark(Name);
                        room.SetPlayerStringMark(ask_who, Name, string.Format("+{0}", ask_who.GetMark(Name)));
                    }
                }
            }
            else if (triggerEvent == TriggerEvent.DamageCaused && data is DamageStruct damage)
            {
                damage.Damage += ask_who.GetMark(Name);
    ...
    public class Qinghuang : TriggerSkill
    {
        public Qinghuang() : base("qinghuang")
        {
            events = new List<TriggerEvent> { TriggerEvent.EventPhaseStart, TriggerEvent.EventPhaseChanging };
        }
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.EventPhaseChanging && data is PhaseChangeStruct change && change.From == PlayerPhase.Play)
                player.SetFlags("-qinghuang");
        }

        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.EventPhaseStart && base.Triggerable(player, room) && player.Phase == PlayerPhase.Play && player.MaxHp > 0)
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
