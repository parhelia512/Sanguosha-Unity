    ...
    public class YiguiProhibt : ProhibitSkill
    {
        ...
        public override bool IsProhibited(Room room, Player from, Player to, WrappedCard card, List<Player> others = null)
        {
            if (card.IsVirtualCard() && card.GetSkillName() == "yigui" && to != null && to.HasShownOneGeneral())
            {
                ...
                bool g1 = to.General1Showed && Engine.GetGeneral(to.ActualGeneral1, room.Setting.GameMode).Kingdom.Contains(general.Kingdom[0]);
                bool g2 = to.General2Showed && Engine.GetGeneral(to.ActualGeneral2, room.Setting.GameMode).Kingdom.Contains(general.Kingdom[0]);
