...
namespace SanguoshaServer.Package
{
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            if (targets.Count > 0)
            {
                ...
                if (target != null)
                {
                    ...
                    {
                        string choice = room.AskForChoice(player, "sanyao_jx", "hp+hancard", new List<string> { "@sanyao:" + target.Name }, target, card_use.Card.SkillPosition);
                        if (choice == "hp")
    ...
    public class XuanhuoJX : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            room.AskForUseCard(player, RespondType.Skill, "@@xuanhuo_jx", "@xuanhuo_jx", null, -1, HandlingMethod.MethodUse, true, info.SkillPosition);
            return new TriggerStruct();
        }
    }
    ...
    public class XuanhuoJXCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            {
                if (room.AskForUseCard(slasher, RespondType.Skill, "@@xuanhuo_jx", string.Format("@xuanhuo-slash:{0}:{1}:", player.Name, victim.Name), null) == null)
                {
    ...
    public class Fumian : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (triggerEvent == TriggerEvent.EventPhaseStart)
            {
                ...
                string choice = room.AskForChoice(player, Name, "draw+target+cancel", prompts, null, info.SkillPosition);
                bool invoke = false;
                if (choice == "draw")
                {
    ...
    public class DuliangCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            if (room.AskForChoice(player, "duliang", "view+more", new List<string> { "@duliang:" + target.Name }, target, card_use.Card.SkillPosition) == "view")
            {
    ...
    public class FuhunVS : ViewAsSkill
    {
        ...
        public override bool IsEnabledAtResponse(Room room, Player player, RespondType respond, string pattern) => MatchSlash(respond);
        public override bool ViewFilter(Room room, List<WrappedCard> selected, WrappedCard to_select, Player player)
        {
    ...
    public class Jujian : PhaseChangeSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            room.AskForUseCard(player, RespondType.Skill, "@@jujian", "@jujian", null, -1, HandlingMethod.MethodUse, true, info.SkillPosition);
            return new TriggerStruct();
        }
    ...
    public class Benxi : TriggerSkill
    {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            else if (triggerEvent == TriggerEvent.CardUsedAnnounced && data is CardUseStruct use)
            {
                ...
                while (count < 2)
                {
                    string choice = room.AskForChoice(player, Name, string.Join("+", choices), new List<string> { "@benxi:::" + use.Card.Name }, data, info.SkillPosition);
                    LogMessage log = new LogMessage
                    {
                        From = player.Name,
                        Card_str = RoomLogic.CardToString(room, use.Card)
    ...
    public class XiansiViewAs : ViewAsSkill
    {
        public XiansiViewAs() : base("xiansi")
        {
            attached_lord_skill = true;
            expand_pile = "revolt";
        }

        public override bool ViewFilter(Room room, List<WrappedCard> selected, WrappedCard to_select, Player player) => selected.Count == 0 && player.GetPile("revolt").Contains(to_select.Id);
        public override bool IsEnabledAtPlay(Room room, Player player) => player.GetPile("revolt").Count > player.Hp && Slash.IsAvailable(room, player);
        public override bool IsEnabledAtResponse(Room room, Player player, RespondType respond, string pattern) => MatchSlash(respond) && room.GetRoomState().GetCurrentCardUseReason() == CardUseReason.CARD_USE_REASON_RESPONSE_USE && player.GetPile("revolt").Count > player.Hp;
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            if (cards.Count == 1)
            {
    ...
    public class XiansiVS : ViewAsSkill
    {
        ...
        {
            if (MatchSlash(respond) && room.GetRoomState().GetCurrentCardUseReason() == CardUseReason.CARD_USE_REASON_RESPONSE_USE)
            {
                Player zhangjiao = RoomLogic.FindPlayerBySkillName(room, "xiansi");
                if (zhangjiao != null && zhangjiao.GetPile("revolt").Count >= 2 )
                    return true;
    ...
    public class XiansiTar : TargetModSkill
    {
        ...
        public override bool CheckSpecificTarget(Room room, Player from, Player to, WrappedCard card) => card.GetSkillName() == "xiansi" && card.SubCards.Count == 2 && RoomLogic.PlayerHasSkill(room, to, "xiansi");
    }
    ...
    public class XiansiCard : SkillCard
    {
        ...
        public override WrappedCard Validate(Room room, CardUseStruct use)
        {
            ...
            room.MoveCardTo(use.Card, null, Place.DiscardPile, new CardMoveReason(MoveReason.S_REASON_NATURAL_ENTER, use.From.Name, target.Name, "xiansi", string.Empty));
                ...
                if (slash.Count > 0)
                {
                    ...
                    if (target.Alive && red && player.Alive)
                    {
                        ...
                        if (targets.Count > 0)
                        {
                            ...
                            WrappedCard used = room.AskForUseCard(target, RespondType.Slash, "Slash:zhongyong", "@zhongyong-slash:" + player.Name, null, -1, HandlingMethod.MethodUse, false);
                            if (used == null)
                            {
                                target.SetFlags("-slashTargetFix");
                                foreach (Player p in targets)
    ...
    public class JianyingRecord : TriggerSkill
    {
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if ((triggerEvent == TriggerEvent.CardUsed || triggerEvent == TriggerEvent.CardResponded) && player.Phase == PlayerPhase.Play && base.Triggerable(player, room))
            {
                WrappedCard card = null;
                if (triggerEvent == TriggerEvent.CardUsed && data is CardUseStruct use)
                    card = use.Card;
    ...
    public class Shibei : TriggerSkill
    {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.Damaged && base.Triggerable(player, room) && room.Current != null && player.GetMark(Name) > 0 && player.GetMark(Name) < 2)
                return new TriggerStruct(Name, player);
    ...
    public class Jigong : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            string choice = room.AskForChoice(player, Name, "3+2+1+cancel", new List<string> { "@jigong" }, null, info.SkillPosition);
            if (choice != "cancel")
            {
    ...
    public class Shifei : ZeroCardViewAsSkill
    {
        ...
        public override bool IsEnabledAtResponse(Room room, Player player, RespondType respond, string pattern)
        {
            string response_id = room.GetRoomState().GetCurrentResponseID().ToString();
            Player current = room.Current;
            if (MatchJink(respond) && !player.HasFlag("shifei_" + response_id) && current != null)
            {
    ...
    */
    ...
    public class Fencheng : ViewAsSkill
    {
        ...
        public override bool IsAvailable(Room room, Player invoker, CardUseStruct.CardUseReason reason, RespondType respond, string pattern, string position = null)
        {
    ...
    public class FenchengCard : SkillCard
    {
        ...
        public override void OnEffect(Room room, CardEffectStruct effect)
        {
            ...
            {
                int count = (int)room.GetTag("fencheng");
                WrappedCard card = room.AskForUseCard(target, RespondType.Skill, "@@fencheng", string.Format("@fencheng:{0}::{1}", effect.From.Name, count), null, -1, HandlingMethod.MethodDiscard);
                if (card != null)
                {
    ...
    public class Shenduan : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            while (ids.Count > 0)
            {
                ...
                WrappedCard card = room.AskForUseCard(ask_who, RespondType.Skill, "@@shenduan", "@shenduan-use", null, -1, HandlingMethod.MethodUse, false, info.SkillPosition);
    ...
    public class HuomoVS : ViewAsSkill
    {
        ...
        public override bool IsEnabledAtResponse(Room room, Player player, RespondType respond, string pattern)
        {
            if (player.IsNude() || !MatchBasic(respond) || room.GetRoomState().GetCurrentCardUseReason() != CardUseReason.CARD_USE_REASON_RESPONSE_USE) return false;
            bool invoke = false;
    ...
    public class Miji : TriggerSkill
    {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            while (give > 0)
            {
                ...
                WrappedCard card = room.AskForUseCard(player, RespondType.Skill, pattern, prompt, null, -1, HandlingMethod.MethodNone, true, info.SkillPosition);
                if (card != null)
                {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            if (!damage)
            {
                ...
                if (player.HasFlag("paiyi_draw") || (!player.HasFlag("paiyi_damage")
                    && room.AskForChoice(player, "paiyi", "draw+damage", new List<string> { "@to-player:" + target.Name, "@paiyi:::" + count.ToString() }, target, card_use.Card.SkillPosition) == "damage"))
                    damage = true;
            }
    ...
    public class Renxin : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (room.AskForUseCard(ask_who, RespondType.Skill, "@@renxin", "@renxin:" + player.Name, null, -1, HandlingMethod.MethodDiscard, true, info.SkillPosition) != null)
            {
                room.NotifySkillInvoked(player, Name);
                room.BroadcastSkillInvoke(Name, ask_who, info.SkillPosition);
                return info;
    ...
    */
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player target, ref object data, Player ask_who, TriggerStruct info)
        {
            if (data is CardUseStruct use)
            {
                ...
                if (target.HandcardNum >= count && count > 0)
                {
                    ...
                    room.SetTag(Name, data);
                    WrappedCard card = room.AskForUseCard(target, RespondType.Skill, "@@qingxi", string.Format("@qingxi:{0}::{1}:{2}", ask_who.Name, use.Card.Name, count), null);
                    room.RemoveTag(Name);
    ...
    public class Jiangchi : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            string choice = room.AskForChoice(player, Name, "more2+more+less+cancel", null, null, info.SkillPosition);
    ...
    public class Sidi : TriggerSkill
    {
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            ...
            {
                FunctionCard fcard = Engine.GetFunctionCard(use.Card.Name);
                if (fcard is Slash && player.ContainsTag("sidi_slash"))
                    player.RemoveTag("sidi_slash");
                else if (fcard.IsNDTrick() && player.ContainsTag("sidi_trick"))
                    player.RemoveTag("sidi_trick");
            }
            else if (triggerEvent == TriggerEvent.EventPhaseEnd && player.Phase == PlayerPhase.Play)
            {
                if (player.ContainsTag("sidi_slash"))
                    player.RemoveTag("sidi_slash");
                if (player.ContainsTag("sidi_trick"))
                    player.RemoveTag("sidi_trick");
            }
            else if (triggerEvent == TriggerEvent.EventPhaseChanging && data is PhaseChangeStruct change && change.From == PlayerPhase.Play
                && player.ContainsTag("sidi_color") && player.GetTag("sidi_color") is List<string> colors)
            {
                player.RemoveTag("sidi_color");
                RoomLogic.RemovePlayerCardLimitation(player, Name);
                foreach (string color in colors)
                    room.RemovePlayerMark(player, "@qianxi_" + color);
            }
        }
        ...
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            ...
            if (triggerEvent == TriggerEvent.EventPhaseStart)
            {
                ...
                {
                    foreach (Player p in RoomLogic.FindPlayersBySkillName(room, Name))
                        if (p != player && p.GetPile(Name).Count > 0)
                            triggers.Add(new TriggerStruct(Name, p));
                }
                else if (player.Phase == PlayerPhase.Finish && base.Triggerable(player, room) && !player.IsNude())
                    triggers.Add(new TriggerStruct(Name, player));
            }
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (player.Phase == PlayerPhase.Finish)
            {
                List<int> ids = room.AskForExchange(player, Name, 1, 0, "@sidi-put", string.Empty, "^BasicCard", info.SkillPosition);
                if (ids.Count > 0)
                {
                    room.NotifySkillInvoked(player, Name);
                    GeneralSkin gsk = RoomLogic.GetGeneralSkin(room, player, Name, info.SkillPosition);
            ...
            {
                List<int> ids = room.AskForExchange(ask_who, Name, 1, 0, "@sidi:" + player.Name, Name, ".|.|.|sidi", info.SkillPosition);
                if (ids.Count == 1)
                {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player target, ref object data, Player ask_who, TriggerStruct info)
        {
            if (target.Phase == PlayerPhase.Play)
            {
                ...
                List<string> colors = target.ContainsTag("sidi_color") ? (List<string>)target.GetTag("sidi_color") : new List<string>();
                if (!colors.Contains(color)) colors.Add(color);
                target.SetTag("sidi_color", colors);
                names.Add(ask_who.Name);
                target.SetTag("sidi_slash", names);
                target.SetTag("sidi_trick", names);
            }
    ...
    public class SidiSlash : TriggerSkill
    {
        ...
        {
            List<TriggerStruct> triggers = new List<TriggerStruct>();
            if (player.Alive && player.Phase == PlayerPhase.Play)
            {
                if (player.ContainsTag("sidi_slash") && player.GetTag("sidi_slash") is List<string> names)
                {
                    List<Player> targets = new List<Player>();
                    foreach (string name in names)
                    {
                ...
                else if (player.ContainsTag("sidi_trick") && player.GetTag("sidi_trick") is List<string> _names)
                {
                    List<Player> targets = new List<Player>();
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (player.ContainsTag("sidi_slash") && ask_who.Alive && player.Alive)
            {
            ...
            if (player.ContainsTag("sidi_trick") && ask_who.Alive)
            {
                room.SendCompulsoryTriggerLog(ask_who, "sidi");

                GeneralSkin gsk = RoomLogic.GetGeneralSkin(room, ask_who, "sidi", info.SkillPosition);
                room.BroadcastSkillInvoke("sidi", "male", 2, gsk.General, gsk.SkinId);
                room.DrawCards(ask_who, 2, "sidi");
            }
    ...
    public class PindiCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            string choice = room.AskForChoice(player, "pindi", "draw+discard", new List<string> { "@to-player:" + target.Name }, target, card_use.Card.SkillPosition);
            target.SetFlags("-pindi_target");
            if (choice == "draw")
    ...
    public class ZhongjianCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            string choice = room.AskForChoice(player, "zhongjian", "discard+draw", new List<string> { "@zhongjian-choice:" + target.Name }, target, card_use.Card.SkillPosition);
            target.SetFlags("-zhongjian_tar");
    ...
    public class Yuce : TriggerSkill
    {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (data is DamageStruct damage && room.GetTag(Name) is List<int> ids)
            {
                ...
                room.SetTag(Name, data);
                WrappedCard discard = room.AskForCard(damage.From, Name, RespondType.None, pattern, string.Format("@yuce-discard:{0}::{1}", player.Name, type), null, HandlingMethod.MethodDiscard, player);
                room.RemoveTag(Name);
    ...
    public class JunxingCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            if (types.Count >= 3
                || room.AskForCard(target, "junxing", RespondType.None, pattern, string.Format("@junxing-discard:{0}", player.Name), null, HandlingMethod.MethodDiscard, player) == null)
            {
                room.TurnOver(target);
    ...
    public class Liandui : TriggerSkill
    {
        public Liandui() : base("liandui")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardUsed, TriggerEvent.CardResponded };
            skill_type = SkillType.Replenish;
        }
        ...
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            List<TriggerStruct> triggers = new List<TriggerStruct>();
            Player last = null;
            if (triggerEvent == TriggerEvent.CardUsed && data is CardUseStruct use && !Engine.IsSkillCard(use.Card.Name) && room.ContainsTag(Name) && room.GetTag(Name) is Player user)
            {
                last = user;
            }
            else if (triggerEvent == TriggerEvent.CardResponded && data is CardResponseStruct resp && resp.Use && room.ContainsTag(Name) && room.GetTag(Name) is Player _user)
            {
                last = _user;
            }
            if (last != null && last.Alive && last != player && base.Triggerable(player, room)) triggers.Add(new TriggerStruct(Name, player));
            if (base.Triggerable(last, room) && player != null && player.Alive && player != last) triggers.Add(new TriggerStruct(Name, player, last));
            return triggers;
        }

        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (room.GetTag(Name) is Player last)
            {
                if (ask_who != player)
                {
                    if (room.AskForSkillInvoke(ask_who, Name, "@liandui-let:" + player.Name))
                    {
                        room.DoAnimate(AnimateType.S_ANIMATE_INDICATE, ask_who.Name, player.Name);
                        room.NotifySkillInvoked(player, Name);
                ...
                else if (room.AskForSkillInvoke(ask_who, Name, last, info.SkillPosition))
                {
                    room.DoAnimate(AnimateType.S_ANIMATE_INDICATE, player.Name, last.Name);
                    room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
                    return info;
                }
            }
        ...
        {
            if (room.GetTag(Name) is Player last)
                room.DrawCards(last, new DrawCardStruct(2, ask_who, Name));
            return false;
        }
    }

    public class LianduiRecord : TriggerSkill
    {
        public LianduiRecord() : base("#liandui")
        {
            events = new List<TriggerEvent> { TriggerEvent.CardUsed, TriggerEvent.CardResponded };
            skill_type = SkillType.Replenish;
        }
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (triggerEvent == TriggerEvent.CardUsed && player != null && data is CardUseStruct use && !Engine.IsSkillCard(use.Card.Name))
                room.SetTag("liandui", player);
            else if (triggerEvent == TriggerEvent.CardResponded && player != null && data is CardResponseStruct resp && resp.Use)
                room.SetTag("liandui", player);
        }
    ...
    public class Biejun : TriggerSkill
    {
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            ...
            {
                if (move.To != null && move.To_place == Place.PlaceHand && move.Reason.SkillName == Name && move.Reason.Reason == MoveReason.S_REASON_GIVE)
                {
                    foreach (int id in move.Card_ids)
                        room.SetCardFlag(id, Name);
                    move.To.SetFlags(Name);
                }
                else if (move.From != null && move.From.HasFlag(Name) && move.From_places.Contains(Place.PlaceHand))
                {
                    foreach (int id in move.Card_ids)
                    {
                        if (room.GetCard(id).HasFlag(Name))
                        {
                            room.SetCardFlag(id, "-biejun");
                            move.From.SetFlags("-biejun");
                            break;
                        }
                    }
                }
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.DamageInflicted && base.Triggerable(player, room) && !player.HasFlag(Name) && player.GetMark(Name) == 1)
                return new TriggerStruct(Name, player);
            return new TriggerStruct();
        }
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            {
                room.SetTag(Name, data);
                bool invoke = room.AskForSkillInvoke(player, Name, string.Format("@biejun:::{0}", damage.Damage), info.SkillPosition);
                room.RemoveTag(Name);
                if (invoke)
                {
                    room.BroadcastSkillInvoke(Name, player, info.SkillPosition);
    ...
    public class BiejunVS : OneCardViewAsSkill
    {
        public BiejunVS() : base("biejun_vs")
        {
            attached_lord_skill = true;
            filter_pattern = ".";
        }
        public override bool IsEnabledAtPlay(Room room, Player player) => !player.IsKongcheng() && !player.HasUsed(BiejunCard.ClassName);

        public override WrappedCard ViewAs(Room room, WrappedCard card, Player player)
        {
            WrappedCard bj = new WrappedCard(BiejunCard.ClassName);
            bj.AddSubCard(card);
            return bj;
        }
    }

    public class BiejunCard : SkillCard
    {
        public static string ClassName = "BiejunCard";
        public BiejunCard() : base(ClassName)
        {
            will_throw = false;
        }
        ...
        public override bool TargetFilter(Room room, List<Player> targets, Player to_select, Player Self, WrappedCard card)
        {
            List<Player> lws = RoomLogic.FindPlayersBySkillName(room, "biejun");
            if (lws.Count <= 1)
                return false;
            else
                return targets.Count == 0 && lws.Contains(to_select) && to_select != Self;
        }

        public override bool TargetsFeasible(Room room, List<Player> targets, Player Self, WrappedCard card)
        {
            List<Player> lws = RoomLogic.FindPlayersBySkillName(room, "biejun");
            return (lws.Count == 1 && targets.Count == 0) || (targets.Count == 1 && lws.Count > 1); 
        }
        public override void OnUse(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From, target = null;
            if (card_use.To.Count == 0)
                target = RoomLogic.FindPlayersBySkillName(room, "biejun")[0];
            else
                target = card_use.To[0];

            room.ObtainCard(target, card_use.Card, new CardMoveReason(MoveReason.S_REASON_GIVE, player.Name, target.Name, "biejun", string.Empty), true);
        }
    }
    ...
    public class Shouxi : TriggerSkill
    {
        ...
        public override TriggerStruct Cost(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (data is CardUseStruct use && use.From.Alive)
            {
                ...
                choices.Add("cancel");
                string choice = room.AskForChoice(player, Name, string.Join("+", choices), new List<string> { string.Format("@shouxi:{0}", use.From.Name) }, data, info.SkillPosition);
                if (choice != "cancel")
                {
    ...
    public class Huaiyi : TriggerSkill
    {
        public Huaiyi() : base("huaiyi")
        {
            events.Add(TriggerEvent.EventPhaseChanging);
            view_as_skill = new HuaiyiVS();
        }

        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (data is PhaseChangeStruct change && change.From == PlayerPhase.Play && player.HasFlag(Name))
                player.SetFlags("-huaiyi");
        }

        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data) => new List<TriggerStruct>();
    }

    public class HuaiyiVS : ZeroCardViewAsSkill
    {
        public HuaiyiVS() : base("huaiyi")
        {
        }

        public override bool IsEnabledAtPlay(Room room, Player player)
        {
            return !player.IsKongcheng() && player.UsedTimes(HuaiyiCard.ClassName) < 1 + (player.HasFlag(Name) ? 1 : 0);
        }
    ...
    public class HuaiyiCard : SkillCard
    {
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            if (!player.IsKongcheng())
            {
                ...
                {
                    List<int> discard;
                    if (room.AskForChoice(player, "huaiyi", "red+black", null, null, card_use.Card.SkillPosition) == "red")
                    {
                        discard = red;
                    }
                    else
                ...
                {
                    player.SetFlags("huaiyi");
                    room.DrawCards(player, 1, "huaiyi");
                }
            }
        }
    }
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            if (data is CardUseStruct use)
            {
                ...
                if (targets.Count > 0)
                {
                    ...
                    WrappedCard card = room.AskForUseCard(target, RespondType.Skill, "@@taoluangive", string.Format("@taoluan-give:{0}::{1}:{2}", player.Name, count, pattern), null);
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (player.Alive)
            {
                ...
                if (suits.Count == 4)
                {
                    ...
                    {
                        WrappedCard card = room.AskForUseCard(player, RespondType.Skill, "@@tongbo", "@tongbo-give", null, -1, HandlingMethod.MethodNone, true, info.SkillPosition);
                        if (card == null && !use)
                            break;
                        else if (card != null)
                        {
    ...
    public class GongqiCard : SkillCard
{
    ...
    public class JiefanCard: SkillCard
    {
        ...
        public override void OnEffect(Room room, CardEffectStruct effect)
        {
            ...
            if (room.AskForCard(effect.To, "jiefan", RespondType.None, pattern, "@jiefan-discard:" + target.Name, effect, HandlingMethod.MethodDiscard, null) == null && target.Alive)
    ...
    public class Zhaofu : ZeroCardViewAsSkill
    {
        ...
        public override WrappedCard ViewAs(Room room, Player player)
        {
            return new WrappedCard(ZhaofuCard.ClassName) { Skill = Name };
        }
    }

    public class ZhaofuCard: SkillCard
    {
        public static string ClassName = "ZhaofuCard";
        public ZhaofuCard() : base(ClassName) { }
        public override bool TargetFilter(Room room, List<Player> targets, Player to_select, Player Self, WrappedCard card)
        {
            return targets.Count <= 1;
        }
        public override void OnUse(Room room, CardUseStruct card_use)
        {
            room.SetPlayerMark(card_use.From, "@zhaofu", 0);
            room.DoSuperLightbox(card_use.From, card_use.Card.SkillPosition, "zhaofu");
            base.OnUse(room, card_use);
        }
        public override void Use(Room room, CardUseStruct card_use)
        {
            Player player = card_use.From;
            player.SetMark("zhaofu_from", 1);
            foreach (Player p in card_use.To)
            {
                p.SetMark("zhaofu_to", 1);
                room.SetPlayerStringMark(p, "zhaofu", string.Empty);
            }
        }
    }

    public class ZhaofuTar : TargetModSkill
    {
        public ZhaofuTar() : base("#zhaofu")
        {
            lord_skill = true;
            pattern = ".";
        }
        ...
        {
            if (from != null && to != null && from.Kingdom == "wu" && from.GetMark("zhaofu_from") == 0 && to.GetMark("zhaofu_to") > 0)
                return true;
            return false;
        }
    }
